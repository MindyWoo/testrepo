...
def get_scatter_chart( entered_site, payload_slider):   
    if entered_site == 'ALL':
        filtered_All_df = spacex_df.loc[ 
                             (spacex_df['Payload Mass (kg)'] >= payload_slider[0]) & 
                             (spacex_df['Payload Mass (kg)'] <= payload_slider[1])]
        fig_scatter_all = px.scatter(filtered_All_df, 
                 x='Payload Mass (kg)', 
                 y='class', 
                 color='Booster Version Category',
                 title=f'Payload Mass vs. Success Rate by Booster Version Category (Payload Range: {payload_slider[0]}-{payload_slider[1]})',                                      
